...
// Inicializa o Resend apenas se a chave de ambiente estiver disponível
  ...
  try {
    ...
    if (type === 'payment' && dataId) {
      ...
      if (paymentData.status === 'approved') {
        ...
        // CORREÇÃO: O Mercado Pago organiza os dados do comprador dentro da estrutura interna do objeto retornado
        const emailComprador = paymentData.payer?.email || 'Não informado';
        const nomeComprador = paymentData.additional_info?.payer?.first_name || paymentData.payer?.first_name || 'Cliente';
        const cpfComprador = paymentData.payer?.identification?.number || 'Não informado';
        ...
        if (process.env.RESEND_API_KEY) {
          ...
          await resend.emails.send({
            from: 'Loja Cordas de Mathias <onboarding@resend.dev>',
            to: 'edson.silva.dev@gmail.com', // ⚠️ IMPORTANTE: Altere para o e-mail onde você quer receber os alertas!
            ...
            <div style="font-family: sans-serif; max-width: 600px; color: #333;">
              ...
              <ul>
                <li><strong>Nome:</strong> ${nomeComprador}</li>
                <li><strong>E-mail:</strong> ${emailComprador}</li>
                <li><strong>WhatsApp:</strong> ${whatsapp}</li>
                <li><strong>CPF:</strong> ${cpfComprador}</li>
              </ul>
